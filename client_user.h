   ...
   返回值2 不存在该用户*/
{
    ...
    recvAddr.sin_port = htons(FILE_PORT);
    recvAddr.sin_addr.s_addr = inet_addr((*ipAddrServer).c_str());
    ...
    int iRes = sendto(ConnectSocket, loginInfo.c_str(), loginInfo.length(), 0, (SOCKADDR*) &recvAddr, sizeof(recvAddr));
    //发送初始登录信息
...
//因为要循环发送多次 防止阻塞
{
   ...
   /*  // for (; ptr != NULL; ptr = ptr->ai_next)
    ...
    int iRes = sendto(ConnectSocket, message, length, 0, (SOCKADDR*) &recvAddr, sizeof(recvAddr));
    //发送初始登录信息
 ...
 */
   /*      iRes = connect(ConnectSocket, ptr->ai_addr, (int)ptr->ai_addrlen);
    ...
    int iRes = sendto(ConnectSocket, message.c_str(), message.length(), 0, (SOCKADDR*) &recvAddr, sizeof(recvAddr));
    //发送初始登录信息
